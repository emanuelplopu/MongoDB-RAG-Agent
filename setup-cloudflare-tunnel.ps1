# RecallHub - Cloudflare Tunnel Setup Script (Multi-Domain)
# This script sets up a Cloudflare Tunnel to expose your local app on multiple domains
# Each tenant gets its own frontend/backend ports mapped to its production domain.
#
...
#   - Zone > DNS > Edit (for all configured domains)
#   - Zone > Zone > Read
#
...
function Show-TokenSetup {
    ...
    Write-Host "  Include > Specific zone > (all configured domains)" -ForegroundColor Yellow
    Write-Host ""
...
function Get-AccountId {
    ...
    try {
        $zoneResponse = Invoke-RestMethod -Uri "https://api.cloudflare.com/client/v4/zones?name=$PRIMARY_DOMAIN" `
            -Headers $headers -Method GET -ErrorAction Stop
...
function New-TunnelConfig {
    ...
    # Build ingress rules dynamically from $DOMAINS config
    $ingressRules = ""
...
"@
    ...
    $config = @"
# Cloudflare Tunnel Configuration (Multi-Domain)
# Auto-generated by setup-cloudflare-tunnel.ps1
# Domains: $($DOMAINS | ForEach-Object { $_.Domain }) -join ", "

tunnel: $TunnelId
credentials-file: $CredentialsPath

# Metrics for monitoring (optional)
metrics: localhost:60124
...
function Start-FullSetup {
    ...
    Write-Host "  Cloudflare Tunnel Setup (Multi-Domain)" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    if ($apiToken) {
        ...
        # Step 4: Setup DNS records for ALL domains
        Write-Step 4 "Configuring DNS Records"
        
        foreach ($domainConfig in $DOMAINS) {
            $domain = $domainConfig.Domain
            $zoneName = if ($domainConfig.Zone) { $domainConfig.Zone } else { $domain }
            Write-Host ""
            Write-Info "Setting up DNS for $domain (zone: $zoneName)..."
            
            $zoneId = Get-ZoneId -ApiToken $apiToken -Domain $zoneName
            if ($zoneId) {
                ...
                Write-Info "You may need to manually configure DNS records for $domain."
            }
        }
    } else {
        ...
        # Setup DNS routes via CLI for all domains
        if ($tunnelId) {
    ...
    Write-Host ""
    Write-Host "1. Start your Docker services (both tenants):" -ForegroundColor White
    Write-Host "   docker compose --profile recallhub --profile quellex up -d" -ForegroundColor Yellow
    Write-Host ""
...
function Install-TunnelService {
    ...
    # Generate system-appropriate config with correct credentials path
    if ($tunnelId) {
...
"@
        ...
        $systemConfig = @"
# Cloudflare Tunnel Configuration - System Service (Multi-Domain)
# Auto-generated by setup-cloudflare-tunnel.ps1
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "For additional monitoring, install the watchdog:" -ForegroundColor Cyan
        Write-Host "  .\scripts\cloudflared-watchdog.ps1 -Install" -ForegroundColor White
    }
}
...
function Show-Help {
    Write-Host @"
Cloudflare Tunnel Setup (Multi-Domain)
=======================================

This script sets up a Cloudflare Tunnel to expose your local app
on multiple production domains, one per tenant.
...
"@
    foreach ($domainConfig in $DOMAINS) {
        ...
        $t = $domainConfig.Tenant
        Write-Host "  $($t):"
        Write-Host "    https://$d         -> localhost:$($domainConfig.FrontendPort) (Frontend)"
        Write-Host "    https://www.$d     -> localhost:$($domainConfig.FrontendPort) (Frontend)"
        Write-Host "    https://api.$d     -> localhost:$($domainConfig.BackendPort) (Backend API)"
    }
    Write-Host @"
...
Required API Token Permissions:
  - Account > Cloudflare Tunnel > Edit
  - Zone > DNS > Edit (for all configured domain zones)
  - Zone > Zone > Read
