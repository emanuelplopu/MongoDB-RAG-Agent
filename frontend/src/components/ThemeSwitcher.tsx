...
export default function ThemeSwitcher({ compact = true }: ThemeSwitcherProps) {
  ...
  const themeOptions: { value: ThemeOption; icon: typeof SunIcon; labelKey: string }[] = [
  ...
  // Get the current display icon based on theme setting
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        title={t(`theme.${theme}`)}
        aria-label={t('theme.switchTo', { mode: theme })}
      >
        <CurrentIcon className="h-5 w-5 text-secondary dark:text-gray-400" />
        {!compact && (
